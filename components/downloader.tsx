 ...
 */
export function Downloader({ locale = "en" }: { locale?: Locale }) {
  const t = T[locale];
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (!cancelledRef.current) {
          setStatus({
            kind: "error",
            message: pdfLoadErrorText(kind, locale) ?? undefined,
          });
        }
      }
    },
    [theme, imageMode, darkness, warmth, darkify, locale],
  );
  ...
  return (
    <div className="w-full max-w-xl mx-auto">
      ...
      <div className="mt-4">
        ...
        {status.kind === "config" && (
          <DropZone onFile={processFile} locale={locale} />
        )}
        ...
        {status.kind === "processing" && (
          <div className="rounded-2xl border-2 border-neutral-800 bg-neutral-900/90 p-10 text-center">
            <div className="text-lg text-neutral-200">
              {status.phase === "render"
                ? t.downloader.convertingPage(status.done, status.total || "?")
                : t.downloader.building}
            </div>
            ...
            <div className="mt-3 text-xs text-neutral-500">
              {t.downloader.runsLocally}
            </div>
            <div className="mt-1 text-xs text-amber-400/80">
              {t.downloader.stayActive}
            </div>
          </div>
        )}
        ...
        {status.kind === "done" && (
          <div className="rounded-2xl border-2 border-neutral-800 bg-neutral-900/90 p-10 text-center">
            ...
            <div className="text-lg font-semibold text-neutral-50">
              {t.downloader.saved(status.filename)}
            </div>
            <div className="mt-1 text-sm text-neutral-400">
              {t.downloader.checkDownloads}
            </div>
            <button
