...
export function HomeComparison({ locale }: { locale: Locale }) {
  ...
  return <section id={locale === "tr" ? "compare-tools" : "why"} className="w-full py-20 border-y border-neutral-900 bg-[#0e0e0e]"><div className="max-w-4xl mx-auto px-6"><h2 className="text-2xl font-bold mb-3 text-center">{copy.title}</h2><p className="text-sm text-neutral-400 text-center mb-10 max-w-xl mx-auto">{copy.intro}</p><div className="hidden sm:block"><table className="w-full text-sm"><thead><tr className="border-b border-neutral-800"><th className="text-left py-3 px-4 font-medium text-neutral-500">{copy.feature}</th><th className="py-3 px-4 font-medium text-amber-400">PDF Dark</th><th className="py-3 px-4 font-medium text-neutral-500">{copy.extensions}</th><th className="py-3 px-4 font-medium text-neutral-500">{copy.others}</th></tr></thead><tbody className="text-center">{copy.rows.map(([feature, dark, extension, other]) => <tr key={feature} className="border-b border-neutral-900"><td className="py-3 px-4 text-left text-neutral-200">{feature}</td><td className="py-3 px-4"><Mark value={dark} yes={yes} no={no} /></td><td className="py-3 px-4"><Mark value={extension} yes={yes} no={no} /></td><td className="py-3 px-4"><Mark value={other} yes={yes} no={no} /></td></tr>)}</tbody></table></div>{/* 移动端：列名只渲染一次并吸顶，避免每张卡片重复三个标签 */}<div className="sm:hidden space-y-3"><div className="sticky top-0 z-10 grid grid-cols-3 gap-2 px-4 py-2 text-xs bg-[#0e0e0e] border-b border-neutral-800"><span className="text-center text-amber-400">PDF Dark</span><span className="text-center text-neutral-500">{copy.mobileExtensions}</span><span className="text-center text-neutral-500">{copy.mobileOthers}</span></div>{copy.rows.map(([feature, dark, extension, other]) => <div key={feature} className="p-4 rounded-xl border border-neutral-800 bg-neutral-900/40"><p className="font-medium text-base text-neutral-100 m-0 mb-3">{feature}</p><div className="grid grid-cols-3 gap-2 text-base text-center"><span className="text-amber-400"><Mark value={dark} yes={yes} no={no} /></span><span><Mark value={extension} yes={yes} no={no} /></span><span><Mark value={other} yes={yes} no={no} /></span></div></div>)}</div></div></section>;
}
